...
QString BaiduEngine::WrapperAPIRequest(QString from, QString to, QString src_text)
{
    ...
    QString params =
        QString("?q=%1&from=%2&to=%3&appid=%4&salt=%5&sign=%6")
            .arg(query_byte.toPercentEncoding(), from.toUtf8(), to.toUtf8(), appid_byte, salt_byte, sign_byte);

    return QString("https://fanyi-api.baidu.com/api/trans/vip/translate") + params;
}
